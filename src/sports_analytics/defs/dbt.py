...
class CustomizedDagsterDbtTranslator(DagsterDbtTranslator):
    ...
    def get_asset_key(self, dbt_resource_props):
        """
        Map dbt resource properties to a Dagster AssetKey.

        Parameters:
            dbt_resource_props (dict): Dictionary of dbt resource properties; expected to contain at least "resource_type" and "name".

        Returns:
            dg.AssetKey: The AssetKey for the corresponding Dagster asset. For resources with "resource_type" equal to "source" the key is "<name>"; otherwise returns the asset key derived from the provided properties.
        """
